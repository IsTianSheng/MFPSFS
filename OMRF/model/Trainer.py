...
from new_.Model.Fusion.epoch_10FPS_FNS import epoch_random_FPS



def trainer(dataset, testSet, trainVector_, trainMaskVector, \
                TopN = [3, 5, 10], epochCount = 700, save_model = False):
    pro_ZR = 50
    pro_PM = 50
    alpha = 0.2
                ...
                if save_model:
                    if topN == 0 and epoch >50:  # top5
                        if model_download < np.mean(P):
                            ...
                            torch.save({'G': G.state_dict()}, save_model)
